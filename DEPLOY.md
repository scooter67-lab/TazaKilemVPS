...
- [ ] **Вынести копии БД за пределы VPS** — ежедневные дампы делаются
      ...
      данных. Канал подойдёт любой: 24.08.2026 исходящие с сервера проверены —
      github.com и api.github.com отдают 200, `git ls-remote` проходит, ghcr.io
      отвечает 401 без авторизации, ACME-директория Let's Encrypt — 200.
      Раздел 0 описывает фильтрацию как действующую и в этой части устарел.
